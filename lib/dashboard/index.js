...
<script>
// ── Embedded data ──
var RAW_RECORDS = ${JSON.stringify(embedRecords)};
var RAW_SUBSCRIPTIONS = ${JSON.stringify(embedSubs)};
...
function renderOverview() {
  ...
  if (localOnly) {
    wb.innerHTML = '<div class="warning-banner"><strong>\\u26a0 Estimated costs</strong> \\u2014 No API keys found. Costs are rough estimates. Set <code>OPENROUTER_API_KEY</code> or <code>ANTHROPIC_ADMIN_KEY</code> for actual billing data.</div>';
  } else { wb.innerHTML = ''; }
  ...
  var mrHTML = top6.map(function(m, i) {
    ...
    var costDisplay = (m.sources === 'local-usage-only') ? '\\u2014' : fmtMoney(m.cost);

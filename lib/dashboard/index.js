...
function generateDashboard(records, sources, allEstimated, subscriptionRecords = [], budgets = [], version = '4.0.0') {
  ...
  const COLORS = "['#c084fc','#60a5fa','#34d399','#f472b6','#fbbf24','#e879f9','#a78bfa','#38bdf8','#fb923c','#4ade80']";
...
<aside class="sidebar">
  ...
  <div class="nav-section">
    ...
    <a class="nav-item" data-view="help" onclick="switchView('help')"><span class="nav-dot"></span> Help / About</a>
  </div>
</aside>
<!-- ── Main Content ── -->
...
var billingPathNames = {
  ...
  'chatgpt-subscription': 'ChatGPT (subscription)', 'copilot-subscription': 'GitHub Copilot (subscription)',
  'windsurf-subscription': 'Windsurf (subscription)',
...
function filterByPeriod(records, days) {
  ...
  return records.filter(function(r) {
    if (r.d < cutoffStr || !enabledSources[r.bp] || r.dup) return false;
    if (currentProject !== 'all' && r.proj && r.proj !== currentProject) return false;
    return true;
  });
}
...
function renderOverview() {
  ...
  var hasAutoSubs = RAW_SUBSCRIPTIONS.some(function(s) { return s.auto; });
  if (hasAutoSubs) {
    warnings.push('<div class="warning-banner" style="border-color:#e879f9;background:rgba(232,121,249,0.06)"><strong>\\u2728 Subscriptions auto-detected</strong> \\u2014 Inferred from your usage data. Run <code>tokenmiser config --add-sub</code> to customize plan details and costs.</div>');
  } else {
    var subBillingPaths = ['oauth', 'cursor-subscription', 'google-subscription', 'chatgpt-subscription'];

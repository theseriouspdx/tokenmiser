    ...
    for (const e of entries) {
      ...
      if (e.isDirectory() && !e.name.startsWith('.')) {
        ...
        try {
          ...
          if (stat.mtimeMs > Date.now() - 90 * 24 * 60 * 60 * 1000) {
            results.push(full);
          }
        } catch {}
      }
...
function collect() {
  ...
  for (const file of jsonlFiles) {
    try {
      // Extract project name from path: ~/.claude/projects/{encoded-path}/{conversation}.jsonl
      // The encoded path segment often contains the project directory name
      const relPath = file.replace(path.join(HOME, '.claude', 'projects') + path.sep, '');
      const pathSegment = relPath.split(path.sep)[0] || '';
      // Decode: path segments use - as separator, last meaningful segment is usually the project name
      const decodedParts = pathSegment.split('-').filter(Boolean);
      const fileProject = decodedParts[decodedParts.length - 1] || '';
      ...
      for (const line of lines) {
        try {
          ...
          // Key includes billing path, entrypoint, and project so they stay separate
          const key = `${date}|${model}|${billingPath}|${entrypoint}|${fileProject}`;
          if (!dayModel[key]) {
            dayModel[key] = { date, model, billingPath, entrypoint, project: fileProject, promptTokens: 0, completionTokens: 0, requests: 0 };
          }

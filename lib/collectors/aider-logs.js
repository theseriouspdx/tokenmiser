...
function collect() {
    ...
    try {
      ...
      for (const [key, agg] of Object.entries(dayModel)) {
        if (!allDayModel[key]) {
          ...
          allDayModel[key].promptTokens += agg.promptTokens;
          allDayModel[key].completionTokens += agg.completionTokens;
          allDayModel[key].cost += agg.cost;
          allDayModel[key].requests += agg.requests;
        }
      }
    } catch {}
  }
  ...
  const records = Object.values(allDayModel).map((agg) => {
    ...
    return {
      ...
      type: 'local-log',
      estimated: agg.cost === 0, // Not estimated if aider provided cost
      date: agg.date,

...
function extractComposerData(dbPath) {
  ...
  if (rows.length === 0) {
    ...
    for (const row of itemRows) {
      ...
      try {
        ...
        extractFromChatData(data, records, thirtyDaysAgo);
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const value = valueRows[0];
      const data = JSON.parse(value);
      extractFromComposer(data, records, thirtyDaysAgo);
    } catch { /* skip unparseable entries */ }
  }
...
function extractFromComposer(data, records, thirtyDaysAgo) {
...
function extractFromChatData(data, records, thirtyDaysAgo) {
